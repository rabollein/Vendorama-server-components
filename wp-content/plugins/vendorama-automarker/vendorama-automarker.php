...
// Schema Markup für Automaten-Einträge (SEO)
add_action('wp_head', 'vendorama_schema_markup');
function vendorama_schema_markup() {
    if (!is_singular('automat')) return;
    $post_id = get_the_ID();
    $lat     = get_field('gps_lat', $post_id);
    $lng     = get_field('gps_lng', $post_id);
    $desc    = get_field('standort_beschreibung', $post_id);
    ...
    // Foto nur ergänzen wenn vorhanden
    if (!empty($foto)) {
        $image_url = is_array($foto) ? $foto['url'] : $foto;
